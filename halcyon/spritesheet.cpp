...
{
    HALCYON_VERIFY(frame_size.x <= tx.size().x && frame_size.y <= tx.size().y, "Spritesheet: Texture size < frame size");
    ...
    HALCYON_PRINT(debug::info, "Created new spritesheet (texture size ", tx.size().x, 'x', tx.size().y, ", frame size ", frame_size.x, 'x', frame_size.y, ')');
}
