...
void console::draw(const font &fnt, const window &wnd) noexcept {
  ...
  for (const auto &entry : m_queue) {
    tex = fnt.render(entry.first, static_cast<color::predefined>(entry.second));

    const pixel_size view{tex.vh(view_height_percentage)};

    tex.draw({offset.x, offset.y + current_height, position_type(view.x),
              position_type(view.y)});
